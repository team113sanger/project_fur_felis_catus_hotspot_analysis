...
def read_file_list(list_file, file_type="MAF"):
    """
...
def is_supported_variant_type(variant_type):
    """
...
def adjust_positions(variant_type, start_position, end_position):
    """
    ...
    try:
        ...
        if not is_supported_variant_type(variant_type):
...
def extract_fields(columns, header_index):
    """
...
def construct_variant_key(fields, adjusted_positions):
    """
...
def construct_variant_info(fields):
    """
...
def extract_variant_info_from_columns(columns, header_index):
    """
    Extracts variant information from MAF file columns.
    Returns variant_key and variant_info if valid, else None.
    """
    fields = extract_fields(columns, header_index)
    if not fields:
        return None
    adjusted_positions = adjust_positions(
        fields["variant_type"], fields["start_position"], fields["end_position"]
    )
    if not adjusted_positions:
        return None
    variant_key = construct_variant_key(fields, adjusted_positions)
    variant_info = construct_variant_info(fields)
...
def add_variant_to_aggregated(variant_key, info, aggregated_variants):
    """
...
def should_skip_line(line, maf_file, line_number):
    """
    ...
    if is_header_line(line):
        ...
        return True
    if not is_valid_line(line, line_number, maf_file):
        LOGGER.debug(f"Skipping invalid or empty line {line_number} in {maf_file}.")
        return True
    return False


def process_line(line, header_index, maf_file, line_number, aggregated_variants):
    """
    Processes a single line from the MAF file.
    Extracts variant information and adds it to the aggregated_variants.
    """
    columns = split_line(line, maf_file, line_number)
    if columns is None:
        return

    variant_info = extract_variant_info_from_columns(columns, header_index)
    if not variant_info:
        return

    variant_key, info = variant_info
    add_variant_to_aggregated(variant_key, info, aggregated_variants)


def process_maf_file(maf_file, header_index, aggregated_variants):
    """
    ...
    try:
        with open(maf_file, "r") as file:
            for line_number, line in enumerate(file, 1):
                if should_skip_line(line, maf_file, line_number):
                    continue
                process_line(
                    line,
                    header_index,
...
def is_valid_line(line, line_number, maf_file):
    """
...
def split_line(line, maf_file, line_number):
    """
...
def aggregate_variants_from_maf_files(maf_files):
    """
    ...
    for maf_file in maf_files:
        header_index = get_headers(maf_file)
        process_maf_file(maf_file, header_index, aggregated_variants)
...
def extract_sample_names(bam_files):
    """
    ...
    for bam_file in bam_files:
        sample_name = get_sample_name_from_bam(bam_file)
        if not sample_name:
...
def run_mpileup_command(command):
    """
...
def extract_alleles_from_key(variant_key):
    """
...
def find_sample_index(sample_name, sample_names):
    """
...
def extract_coverage_sequence(
    mpileup_line, start_column, line_number, variant_key, sample_name
):
    """
...
def check_presence_in_maf(variant_key, sample_name, aggregated_variants):
    """
...
def calculate_status(presence_in_maf, mean_alt_percentage, found_alt, mnp_alleles):
    """
...
def format_alt_percentage(mean_alt_percentage):
    """
...
def process_sample_variant(
    ...
    (
        ...
        reference_allele,
        alternate_allele,
    ) = extract_alleles_from_key(variant_key)
    if chromosome is None:
        return 0, 0, 0, "ERROR"

    sample_index = find_sample_index(sample_name, sample_names)
    ...
    presence_in_maf = check_presence_in_maf(
        variant_key, sample_name, aggregated_variants
    )
    found_alt = count_found_alt(alt_count_list)
    status = calculate_status(
        presence_in_maf, mean_alt_percentage, found_alt, mnp_alleles
    )
...
def process_variants(
    ...
    for variant_key in sorted(aggregated_variants.keys()):
        ...
        mpileup_output = run_mpileup_command(mpileup_command)
        ...
        for sample_name in sample_names:
            ...
            formatted_alt_percentage = format_alt_percentage(mean_alt_percentage)
...
def main(args: argparse.Namespace):
    ...
    check_required_files([bam_list_file, maf_list_file, reference_fasta])

    # Read MAF and BAM files lists
    maf_files = read_file_list(maf_list_file, file_type="MAF")
    bam_files = read_file_list(bam_list_file, file_type="BAM")

    # Aggregate variants from MAF files
    aggregated_variants = aggregate_variants_from_maf_files(maf_files)
    ...
    sample_names = extract_sample_names(bam_files)
